...
import static kom.geometry.core.GpcHelper.polygonClip;

public class Gpc {

    public static GeometryTransfer clip(GpcOperation operation, GeometryTransfer subject, GeometryTransfer clip) {
        return polygonClip(operation.getValue(), subject, clip, false);
    }

    public static GeometryTransfer clip(GpcOperation operation, GeometryTransfer subject, GeometryTransfer clip,
            boolean returnAsTriStrips) {
        return polygonClip(operation.getValue(), subject, clip, returnAsTriStrips);
    }

    public static GeometryTransfer clip(GpcOperation operation, GpcPolygon subject, GpcPolygon clip) {
        return polygonClip(operation.getValue(), new GeometryTransfer(subject),
